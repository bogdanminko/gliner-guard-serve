...
5. **Документация** — описать метод инференса и его особенности (см. ниже)
...
|-----------|-------------|
| Сервер (инференс) | A100 80G PCIe, 14 vCPU |
| Клиент (Locust) | отдельная виртуалка рядом, 16 vCPU |
...
При добавлении нового бенчмарка опишите особенности вашего метода. Что стоит указать:
