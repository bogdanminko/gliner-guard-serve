...
class _CountTransformer(nn.Module):
    """Transformer sub-module used by CountLSTMv2 after GRU unrolling."""

    def __init__(self, hidden_size: int, proj_dim: int = 128,
                 nhead: int = 4, num_layers: int = 2, dim_feedforward: int = 256):
        super().__init__()
        self.in_projector = nn.Linear(hidden_size, proj_dim)
        ...
        self.out_projector = nn.Sequential(
            nn.Linear(proj_dim + hidden_size, hidden_size),
            nn.ReLU(),
    ...
    def forward(self, gru_out: torch.Tensor, pc_emb_expanded: torch.Tensor) -> torch.Tensor:
        """gru_out: (count, M, D), pc_emb_expanded: (count, M, D) → (count, M, D)."""
        proj = self.in_projector(gru_out)
        trans_out = self.transformer(proj)
        return self.out_projector(torch.cat([trans_out, pc_emb_expanded], dim=-1))


class CountLSTMv2(nn.Module):
    """Count-aware unrolling v2: GRU + Transformer refinement."""

    def __init__(self, hidden_size, max_count=20, proj_dim=128,
        ...
        self.pos_embedding = nn.Embedding(max_count, hidden_size)
        self.gru = nn.GRU(input_size=hidden_size, hidden_size=hidden_size)
        self.transformer = _CountTransformer(
            hidden_size=hidden_size, proj_dim=proj_dim,
    ...
    def forward(self, pc_emb: torch.Tensor, gold_count_val: int) -> torch.Tensor:
        ...
        pos_seq = self.pos_embedding(count_indices).unsqueeze(1).expand(gold_count_val, M, D)
        h0 = pc_emb.unsqueeze(0)
        gru_out, _ = self.gru(pos_seq, h0)
        pc_expanded = pc_emb.unsqueeze(0).expand_as(gru_out)
        return self.transformer(gru_out, pc_expanded)
...
class GLiNER2Pooler(nn.Module):
    ...
    def __init__(self, hidden_size: int, max_width: int = 8,
        ...
        # Count embedding
        if counting_layer == "count_lstm_v2":
